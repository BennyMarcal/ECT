package aula12.guiao12.Ex1.AliniaA;
...
// Adicione a seguir o código necessário para a leitura do ficheiro 
		...
		fest = new AgenciaTuristica("Tours4Eating", "endereco");

		try {
			File file = new File("uafest.txt");
			...
			while( sc.hasNextLine() ) {
				...
				int sel = Integer.parseInt(row[0]);
				if( sel == 1 ) {
					Gastronomia g = new Gastronomia(Integer.parseInt(row[1]), row[2]);
					fest.add(g);
				} else {
					PasseioBicicleta pb = new PasseioBicicleta(Integer.parseInt(row[1]), row[2]);
