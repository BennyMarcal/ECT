...
{  
    int *p = (int *) &ADC1BUF0;         //ler o ponteiro da primeira posicao
    int i, average = 0;                 //iniciar variavel

    printInt(p[0], 16 | 3 << 16);       //print na consola
    putChar('\n');

    for (i = 0; i < 8; i++)           //como diz 8 samples no enunciado, o for fica com 8
        average += p[i * 4];          //calcular a média, decorar

    voltage = toBcd(voltageConversion(average / 8));  //converter a voltage, convertemos com a media e a dividir pelas samples
    //voltage = toBcd(average/8) * 33 + 511 / 1023;  //converter sem a funcao de conversion
    
    IFS1bits.AD1IF = 0;                             //resetar a flag
}
