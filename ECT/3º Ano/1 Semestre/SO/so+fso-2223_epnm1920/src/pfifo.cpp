...
void init_pfifo(PriorityFIFO* pfifo)
{
   ...
   pfifo->fifoaccess = PTHREAD_MUTEX_INITIALIZER;
...
void insert_pfifo(PriorityFIFO* pfifo, uint32_t id, uint32_t priority)
{
   ...
   pthread_mutex_lock(&pfifo->fifoaccess);
   while (full_pfifo(pfifo))
   {
      pthread_cond_wait(&pfifo->fifonotfull, &pfifo->fifoaccess);
   }
   require(!full_pfifo(pfifo), "FIFO is full");
   ...
   pthread_cond_broadcast(&pfifo->fifonotempty);
   pthread_mutex_unlock(&pfifo->fifoaccess);
}
...
uint32_t retrieve_pfifo(PriorityFIFO* pfifo)
{
   ...
   mutex_lock(&pfifo->fifoaccess);
   while (empty_pfifo(pfifo))
   {
      pthread_cond_wait(&pfifo->fifoaccess, &pfifo->fifonotempty);
   }
   
   require(!empty_pfifo(pfifo),"FIFO is empty")
   ...
   pthread_cond_broadcast(&pfifo->fifonotfull);
   pthread_mutex_unlock(&pfifo->fifoaccess);
