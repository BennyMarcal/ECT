...
{
    int num_patients;
    Patient all_patients[MAX_PATIENTS+1];
    PriorityFIFO triage_queue;
    PriorityFIFO doctor_queue;
    
    pthread_mutex_t accessPatient[MAX_PATIENTS+1];
    pthread_cond_t hasFinished[MAX_PATIENTS+1];
} HospitalData;
...
void init_simulation(uint32_t np)
{
   ...
   {
      mutex_init(&hd->accessPatient[i], NULL);
      cond_init(&hd->hasFinished[i], NULL);
   }
...
void doctor_iteration()
{
   ...
   cond_signal(&hd->hasFinished[patient]);
...
void patient_goto_urgency(int id)
{
   ...
   printf("\e[33;01mPatient %s (number %u): get to hospital\e[0m\n", hd->all_patients[id].name, id);
...
{
   mutex_lock(&hd->accessPatient[id]);

   while(hd->all_patients[id].done == 0)
   {
      cond_wait(&hd->hasFinished[id], &hd->accessPatient[id]);
   }

   check_valid_name(hd->all_patients[id].name);
   printf("\e[33;01mPatient %s (number %u): health problems treated\e[0m\n", hd->all_patients[id].name, id);

   mutex_unlock(&hd->accessPatient[id]);
}
...
int main(int argc, char *argv[])
{
   ...
   uint32_t patientIds[npatients];
   for(uint32_t i=0; i < npatients; i++)
   {
      patientIds[i] = i;
      thread_create(&patients[i], NULL, patient_thread, &patientIds[i]);
   }
   ...
   for(uint32_t i=0; i < npatients; i++)
   {
      ...
      printf("Patient %u finished\n", i);
   }

   pthread_t dummy;
   uint32_t dummyId = MAX_PATIENTS;
   thread_create(&dummy, NULL, patient_thread, &dummyId);
   thread_join(dummy, NULL);

   /* nurses */
   for(uint32_t i=0; i < nnurses; i++)
   {
      thread_join(nurses[i], NULL);
      printf("Nurses %u finished\n", i);
   }
